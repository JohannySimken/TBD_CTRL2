...
1. Tener el servicio de PostgreSQL instalado y corriendo, con la extensión PostGIS disponible (instalable como paquete adicional en la mayoría de las distribuciones/gestores de paquetes).

2. Crear de la base de datos:
...
4. Configurar la conexión en [backend/src/main/resources/application.properties](backend/src/main/resources/application.properties) para que apunte a tu instancia local (ajusta usuario/contraseña según tu instalación):
...
- Ir a `http://localhost:5173/register`, completar usuario/email/contraseña y hacer clic en el mapa para fijar tu ubicación. Esa ubicación queda guardada en la BD como tu **"punto registrado"**, usado por varias de las consultas estadísticas (por ejemplo, la Sección 8 del dashboard).
- Si ya tienes cuenta (o usas uno de los usuarios de prueba de arriba), inicia sesión en `/login`.
...
- Cada tarea se puede editar, eliminar o marcar como completada desde el listado. Si editas una tarea `Expirada` y le colocas una fecha/hora futura, vuelve automáticamente a `Pendiente`.
...
### 3. Notificaciones (icono de campanita en el header)
...
4. **Promedio de distancia de tareas completadas (punto elegido por vos)** — a diferencia de la Sección 8, acá se elige el punto de referencia: hacer clic en el mapa de esta sección o usando el botón **"Usar mi ubicación actual"** (geolocalización del navegador), y presionar **"Calcular promedio"**. El resultado se muestra en kilómetros.
...
8. **Promedio de distancia de tareas completadas respecto al punto registrado en la BD** — mismo cálculo que la consulta 4, pero siempre contra la ubicación que guardaste al registrarte (no editable desde acá).
